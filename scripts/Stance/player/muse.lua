...
function M.new(ctx)
    ...
    local core  = ctx.core
    -- Simulation clock + last-tick timestamp, so the inspiration buff counts down in
    -- lockstep with the world (slows under slow-motion / time scale, pauses with the
    -- game) instead of on real frame time. nil re-baselines on the first tick / load.
    local function now()
        local t = 0
    ...
    -- The instrument currently being played, e.g. 'Lute'/'Drum'/'Fiddle'/
    -- 'Ocarina'/'BassFlute'/'PanFlute' (Bardcraft's Song.getInstrumentProfile
    -- name) — tracked independently of `perf`, which only exists for IDLE
    -- (Muse-buff-eligible) practice play. The instrument should reflect
    -- whatever the player is visibly holding for ANY performance type (a real
    -- tavern/street gig included), not just idle practice.
    local currentInstrument = nil
    ...
    local function beginPerformance(songId, songTitle, perfType, isComposed)
        ...
        isPerforming = true
        debugLog(string.format('Song: idle performance of "%s" → buffs %s (loops allowed: %d)',
    ...
    local function finishPerformance(completion)
            ...
            if applyBuff(p.stance, bufferTime, p.songTitle) then
                ...
                if musePerkUnlocked(50) then
                    ...
                    if kin and kin ~= p.stance then
                        ...
                        debugLog(string.format('Song Shared Refrain: kindred %s buffed at half for %.0fs',
                            tostring(kin), bufferTime), 'debugPerkMessages')
                    end
                end

                reapplyAllBuffs()
                debugLog(string.format('Song: "%s" → +%d %s inspiration for %.0fs (%d/%d notes)%s',
        ...
        if ui and ui.showMessage
            ...
            if buffed then
                msg = string.format(
                    'Song: "%s" inspires %s — +%d for %s.  (%d/%d notes clean, %d loop%s)',
            ...
            elseif p.stance then
                msg = string.format(
                    'Song: "%s" was too rough to inspire %s.  (%d/%d notes clean)',
                    title, stanceName(p.stance), p.successes or 0, p.notes or 0)
            else
                msg = string.format('Song: "%s" ends. (no stance to inspire)', title)
            end
    ...
    local function onConductorEvent(e)
        ...
        if e.type == 'PerformStart' then
            ...
            currentInstrument = e.instrument
    ...
    local function update(dt)
        dt = tonumber(dt) or 0
        -- Count the inspiration buff down on the SIMULATION clock, not the raw frame
        -- dt, so it tracks slow-motion / pause exactly like engine effect durations.
        local _t = now()
        local simDt = (lastTickAt and math.max(0, math.min(_t - lastTickAt, 0.5))) or 0
        lastTickAt = _t
        local st = getState()
        local changed = false
        local anyLive = false
        for stanceId, buff in pairs(st.activeBuffs) do
            if buff.remaining then
                buff.remaining = buff.remaining - simDt
    ...
    -- The instrument currently being played ('Lute'/'Drum'/'Fiddle'/'Ocarina'/
    -- 'BassFlute'/'PanFlute'), or nil when not performing. Independent of
    -- getPerformanceStatus: this reflects ANY performance, not just
    -- idle/Muse-buff-eligible ones.
    local function getCurrentInstrument()
