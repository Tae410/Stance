...
function M.new(ctx)
    ...
    local lastTickAt = nil          -- getSimulationTime() at the previous update tick
    ...
    -- Landed a PERFECT PARRY with Felthorn (no shield — the caller gates on that).
    -- Feeds a configurable FRACTION of the on-hit resonance to the meter, rewarding
    -- defensive mastery alongside offense. No-op while resonant/exhausted (addMeter
    -- only builds), so it cannot extend an active surge.
    function api.onPerfectParry(activeStanceId)
        if not featureOn() then return end
        if activeStanceId ~= STANCE_ID then return end
        local frac = tonumber(cfg().perfectParryResonanceFraction) or 0.5
        if frac <= 0 then return end
        local perHit = tonumber(cfg().buildPerHit) or 12
        addMeter(perHit * frac, activeStanceId)
        debugLog(string.format('Blademeister: perfect parry fed %.1f resonance (%.0f%% of on-hit).',
            perHit * frac, frac * 100), 'debugPerkMessages')
    end

    -- True while the Soul Resonance surge is active (used by Spellsword Resonance).
    function api.isResonant(activeStanceId)
        if activeStanceId ~= STANCE_ID then return false end
        return getState().phase == 'resonant'
    end
    ...
    function api.update(activeStanceId, dt)
        ...
        -- Drive decay/drain off the SIMULATION clock (getSimulationTime via now()),
        -- not the raw frame dt, so the meter drains in lockstep with the world: it
        -- slows under slow-motion / time scale and pauses with the game, exactly like
        -- the engine's own effect durations. lastTickAt is refreshed every tick
        -- (including the inactive early-return below), so reactivating the stance
        -- never replays a catch-up gap; nil on the first tick / after a load
        -- re-baselines with simDt = 0. Clamped to absorb any hitch or sim-time jump.
        local _t = now()
        ...
        elseif s.phase == 'resonant' then
            ...
            s.meter = (s.meter or 0) - drainRate * simDt
            if s.meter <= 0 then
            ...
            if (now() - lastHitAt) > grace and (s.meter or 0) > 0 then
                local decay = (tonumber(c.decayPerSec) or 4) * simDt
