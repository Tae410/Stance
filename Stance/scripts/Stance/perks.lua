...
local EFF = {
    ...
    FireDamage    = _cme.FireDamage    or 14,
    FrostDamage   = _cme.FrostDamage   or 16,
    ShockDamage   = _cme.ShockDamage   or 21,  -- note: distinct MGEF, set below
}
-- ShockDamage's canonical MGEF index is 21 in some tables and collides with
-- DamageHealth's fallback; prefer the engine's named id and only fall back to the
-- canonical Morrowind ShockDamage index (22 in the standard MGEF order).
EFF.ShockDamage = _cme.ShockDamage or 22
...
local EFF_NAME = {
    ...
    [EFF.FireDamage]     = 'fireDamage',
    [EFF.FrostDamage]    = 'frostDamage',
    [EFF.ShockDamage]    = 'shockDamage',
}
-- Spellsword imbue element id → elemental effect to add on a resonant hit.
local ELEMENT_TO_EFF = {
    firedamage  = EFF.FireDamage,
    frostdamage = EFF.FrostDamage,
    shockdamage = EFF.ShockDamage,
}
...
-- Spellsword Resonance (Phase 3) config; the resonant flag + imbue element arrive
-- on the attack table (computed init-side where those modules are in scope).
local function spellswordResonanceCfg() return C and C.getSpellswordResonanceCfg and C.getSpellswordResonanceCfg() or nil end
...
function Perks.onHit(attack)
    ...
    --
    -- Spellsword Resonance (Phase 3): while RESONANT and carrying a Spellsword
    -- elemental imbue (Blazed/Frozen/Electrified), every hit deals a bonus burst
    -- of the MATCHING element — a fraction of the hit's physical damage. Fully
    -- transient: the imbue element is the gate (so it needs Spellsword active), and
    -- the bonus vanishes the instant resonance ends or the imbue is cleared. Nothing
    -- is stored, so there is nothing to clean up on load.
    if sid == 'blademeister' then
        local src = spellswordResonanceCfg()
        if src and src.enabled ~= false then
            local resonantOK = (src.requiresResonanceState == false) or (attack.blademeisterResonant == true)
            local element    = attack.imbueElement            -- nil unless Spellsword-imbued
            local effId      = element and ELEMENT_TO_EFF[element]
            if resonantOK and effId then
                local frac = tonumber(src.elementalDamageBonus) or 0.30
                -- attack.damage may be a number or a {health=...}/{damage=...} table.
                local dmg, base = attack.damage, 0
                if type(dmg) == 'number' then base = dmg
                elseif type(dmg) == 'table' then
                    base = tonumber(dmg.health) or tonumber(dmg.damage) or 0
                end
                local bonus = base * frac
                if bonus > 0 then sendEffect(tgt, effId, bonus, 0) end
            end
        end
    end
