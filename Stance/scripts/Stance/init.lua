...
local function ensurePerksInit()
    ...
    local perkContext = {
        ...
        -- Spellsword Resonance config (config is in lexical scope here).
        getSpellswordResonanceCfg = function() return config.spellswordResonance end,
    }
...
local INTEGRATION_SETTING_KEY = {
    ...
    eveningstar    = 'integrateEveningStar',
    ossc           = 'integrateOSSC',
    ironfist       = 'integrateIronfist',
}
...
-- ─── Evening Star (Religions of Morrowind) worship bonus ──────────────────
-- Determines the player's Tribunal devotion tier by reading their OWN spells for
-- the abilities Evening Star grants per deity (gift_1 at Worshipper/Follower,
-- gift_3 at Devotee). This needs no Evening Star API. Cached and refreshed on the
-- normal poll (the tier rarely changes) so the per-tick effectiveness refresh that
-- consumes it stays cheap. Covers ONLY the Tribunal Temple deities.
local esTierCache = {}        -- [deityId] = 0 none / 1 Follower / 2 Devotee
local esPresent   = false     -- Evening Star content actually loaded (gift records exist)
...
local function playerHasSpell(spellId)
    ...
    local ok, has = pcall(function()
        ...
        if spells[spellId] ~= nil then return true end      -- fast path: id index
        for _, sp in pairs(spells) do                        -- fallback: scan
            local id = (type(sp) == 'table') and sp.id or sp
...
-- True when Evening Star content is loaded: at least one Tribunal gift ability
-- exists in the spell records. Lets the tooltip association and the bonus stay
-- hidden entirely when the mod is absent (independent of the on/off setting).
local function eveningStarPresent()
    local es = config.eveningStar
    if not (es and es.deities and core.magic and core.magic.spells and core.magic.spells.records) then
        return false
    end
    for _, d in pairs(es.deities) do
        if d.gift1 then
            local ok, rec = pcall(function() return core.magic.spells.records[d.gift1] end)
...
local function refreshWorshipCache()
    local fresh = {}
    local es = config.eveningStar
    esPresent = eveningStarPresent()
    if es and es.deities and esPresent and integrationEnabled('eveningstar') then
        for deityId, d in pairs(es.deities) do
            local tier = 0
            if d.gift3 and playerHasSpell(d.gift3) then tier = 2
            elseif d.gift1 and playerHasSpell(d.gift1) then tier = 1 end
            fresh[deityId] = tier
        end
    end
    esTierCache = fresh
...
-- Returns (tier, deityId) for a stance: tier 0 none / 1 Follower / 2 Devotee.
local function worshipTierForStance(stanceId)
    local es = config.eveningStar
    if not stanceId or not es or not esPresent or not integrationEnabled('eveningstar') then return 0, nil end
    local deityId = es.stanceDeity and es.stanceDeity[stanceId]
    if not deityId then return 0, nil end
    return (esTierCache[deityId] or 0), deityId
end

-- Integer bonus added to the ACTIVE stance's target skill from Tribunal worship
-- (0 when the integration is off, the stance has no deity, or the player does not
-- worship it). Folded into the effectiveness target-skill modifier in
-- refreshEffectivenessModifiers / the modded dynamic-modifier path.
local function worshipSkillBonus()
    local tier = worshipTierForStance(activeStanceId)
    if tier <= 0 then return 0 end
    local es = config.eveningStar
    return math.floor(((es.tierBonus and es.tierBonus[tier]) or 0) + 0.5)
end

-- Tooltip data for a stance's Tribunal association, or nil if it has none / the
-- integration is off. tierName/bonus are nil/0 when the player does not yet worship.
local function getWorshipTooltipInfo(stanceId)
    local es = config.eveningStar
    if not es or not esPresent or not integrationEnabled('eveningstar') then return nil end
    local deityId = es.stanceDeity and es.stanceDeity[stanceId]
    local d = deityId and es.deities and es.deities[deityId]
    if not d then return nil end
    local tier = esTierCache[deityId] or 0
    return {
        deityName    = d.name,
        title        = d.title,
        pantheonName = es.pantheonName,
        tierName     = (tier > 0 and es.tierName and es.tierName[tier]) or nil,
        bonus        = (tier > 0 and math.floor(((es.tierBonus and es.tierBonus[tier]) or 0) + 0.5)) or 0,
    }
end
...
-- Stance Codex: hotkey-toggled info overlay (level/perks/attribute per
-- stance). Attached as a field on xpModule, not a new top-level local --
-- init.lua's main chunk is at Lua's 200-local ceiling.
xpModule.codex = require('scripts.stance.player.codex').new({
    ui = ui, async = async, util = util, input = input, config = config,
    stanceEnabled    = stanceEnabled,
    getStanceLevel   = getStanceLevel,
...
local felthornVoice = require('scripts.stance.player.felthorn_voice').new({
    ...
    core        = core,
    readSetting = readSetting,
    getHealthFraction = function()
        local frac = 1
        pcall(function()
            local h = types.Actor.stats.dynamic.health(self)
            if h and h.modified and h.modified > 0 then
                frac = (h.current or h.modified) / h.modified
            end
        end)
        return frac
    end,
...
local getMuseCurrentInstrument   = muse.getCurrentInstrument
...
local skillFramework = require('scripts.stance.player.skill_framework').new({
    ...
    getMuseCurrentInstrument     = getMuseCurrentInstrument,
    ...
    getActiveSmokeInfo         = prefixes.getActiveSmokeInfo,
    worshipSkillBonus          = worshipSkillBonus,
    getWorshipTooltipInfo      = getWorshipTooltipInfo,
...
-- SneakIsGoodNow integration module (attentiveness + sneak weapon skill bonus)
local sneakIsGood = require('scripts.stance.player.sneakisgood').new({
    self                    = self,
    types                   = types,
    core                    = core,
    config                  = config,
    ...
    getActiveStance         = function() return activeStanceId end,
    getStanceLevel          = getStanceLevel,
    getActiveStanceConfig   = getStanceConfig,
    currentFortifiedBlockBonus = currentFortifiedBlockBonus,
})
...
local SYNCED_KEYS = {
    ...
    -- The only Integrations-group key mirrored so far: victim.lua (a LOCAL
    -- script on NPC/CREATURE, not this player script) needs it to gate the
    -- Brawler+Iron Fist unarmed-damage bonus, and storage.playerSection isn't
    -- reachable from local scope, so Runtime_Stance is the only way it can see
    -- a settings toggle at all. No other integration has needed this before
    -- because every other one is consumed entirely on the player side.
    { 'Integrations', 'integrateIronfist' },
}
...
local function syncSettingsToGlobal(force)
    ...
    -- Brawler's own stance level, mirrored ONLY for victim.lua's Iron Fist
    -- amplification term (see config.brawlerGauntlet) — it needs the
    -- progression fraction but, being a local script, cannot call
    -- getStanceLevel/effectivenessSkillBonus directly the way player-side
    -- code does.
    payload.brawlerLevel = getStanceLevel('brawler')
...
local function drainStanceLevelUps()
    ...
    for _, up in ipairs(ups) do
        ...
        if up.stanceId == 'blademeister' then
            pcall(function() felthornVoice.sayEvent('blademeister', 'levelUp') end)
        end
...
local function onPlayerDealtHit(data)
    ...
    -- Spellsword Resonance (Phase 3): resolve the resonant flag + Spellsword imbue
    -- element here, where blademeister/prefixes are in lexical scope, and pass them
    -- to the perk on-hit handler (which cannot reference those late-bound modules).
    -- Only meaningful for Blademeister; left nil/false otherwise.
    local resonantNow, imbueElementNow = false, nil
    if activeStanceId == 'blademeister' then
        pcall(function() resonantNow = blademeister.isResonant(activeStanceId) end)
        pcall(function() imbueElementNow = prefixes.getImbueElement() end)
    end
    Perks.onHit({
        target = data.target,
        weapon = data.weapon,
        damage = data.damage,   -- for Spellsword Resonance elemental scaling
        blademeisterResonant = resonantNow,
        imbueElement = imbueElementNow,
    })
...
-- Bullseye ranged attack integration (for Huntsman stance).
local integrationsBullseye = require('scripts.stance.player.integrations_bullseye').new({
    self = self, types = types, core = core,
    config = config, Perks = Perks,
    readSetting         = readSetting,
    debugLog            = debugLog,
    getActiveStance     = function() return activeStanceId end,
    getStanceLevel      = getStanceLevel,
    getActiveStanceConfig = getActiveStanceConfig,
    grantStanceXp       = grantStanceXp,
})
local onBullseyeHit = integrationsBullseye.onBullseyeHit
...
local function onUiModeChangedCombined(data)
    ...
    elseif data.oldMode == 'Dialogue' and data.newMode == nil then
        talkDebounce = false
        -- Safety net: don't leave a training-selection menu floating if the
        -- player leaves the trainer mid-choice (walking away, Escape, etc.).
        pcall(function() xpModule.trainers.closeMenu() end)
    end
end
...
local function onUpdate(dt)
    ...
    sneakIsGood.refreshAttentivenessBonus()

    -- Refresh the Evening Star devotion-tier cache (reads the player's granted
    -- worship abilities) so the worship skill bonus folded into the effectiveness
    -- modifier below is current. Cheap and throttled to the poll interval.
    refreshWorshipCache()
    ...
    pcall(xpModule.codex.update, dt)
...
return {
    ...
    interface = {
        ...
        -- ── Saved-loadout classification (for the Loadouts mod) ──────────────
        -- Answer "which stance would this saved loadout be in, and what is its
        -- decorated name?" from stored record ids — no live equipment needed.
        -- Both PLAYER-scope scripts run on the player actor, so the Loadouts
        -- menu reads these straight off I.Stance.
        --
        -- The canonical, one-call entry point. Pass the loadout's stored ids:
        --   opts.rightId   — CarriedRight record id (main weapon / tool), or nil
        --   opts.offHandId — CarriedLeft record id (a shield here → Fortified),
        --                    or nil
        --   opts.secondId  — Dual Wielding off-hand weapon record id (→ Dualist),
        --                    or nil
        -- Returns { id, name, fortified, attribute }:
        --   id        — resolved stance id (e.g. 'soloist')
        --   name      — display name, Fortified-decorated when applicable
        --               (e.g. 'Fortified Soloist'); no imbue/Sneaky, which are
        --               runtime-only and absent from a saved loadout
        --   fortified — whether the Fortified prefix/bonus would apply
        --   attribute — the stance's governing attribute (e.g. 'endurance')
        describeLoadout      = function(opts)
            opts = opts or {}
            local res = classifyRecord({ rightId = opts.rightId, secondId = opts.secondId })
            local id  = (res and res.id) or 'commoner'
            local stance = getStanceConfig(id)
            -- Fortified: a shield stored in the off-hand alongside a fortifiable
            -- one-handed weapon stance, honouring the same enableFortified setting
            -- the live HUD uses. isFortifiable already excludes Dualist and every
            -- non-one-handed-melee stance, so a second-weapon loadout never reads
            -- as Fortified.
            local fortified = false
            if opts.offHandId
                and isFortifiable(id)
                and readSetting('Stances', 'enableFortified', true)
                and isShieldRecord(opts.offHandId) then
                fortified = true
            end
            return {
                id        = id,
                name      = formatLoadoutStanceName(id, fortified),
                fortified = fortified,
                attribute = stance and stance.attribute or nil,
            }
        end,
        -- Lower-level helpers, exposed for completeness / other consumers.
        classifyLoadout      = function(opts) return classifyRecord(opts) end,
        getStanceDisplayName = function(id)
            local s = getStanceConfig(id or activeStanceId)
            return (s and s.displayName) or id
        end,
        isFortifiableStance  = function(id) return isFortifiable(id) end,
        isShieldRecord       = function(recordId) return isShieldRecord(recordId) end,
    },
    engineHandlers = {
        ...
        onKeyPress = function(key) pcall(function() xpModule.codex.onKeyPress(key) end) end,
    },
    eventHandlers = {
        ...
        ngarde_parrySelf            = function(payload)
            onNGardeParrySuccess(payload)
            -- Phase 2: a PERFECT Felthorn parry with NO shield also feeds a fraction
            -- of the on-hit resonance to the Blademeister meter. Inlined (not a
            -- top-level local) to stay under Lua's 200-locals-per-function limit.
            if type(payload) == 'table' and payload.isPerfect == true
                and activeStanceId == 'blademeister'
                and (currentFortifiedBlockBonus() or 0) <= 0 then
                pcall(function() blademeister.onPerfectParry(activeStanceId) end)
                pcall(function() felthornVoice.sayEvent(activeStanceId, 'perfectParry') end)
            end
        end,
        -- Bullseye ranged hit on the player (carries damage multiplier).
        -- Huntsman stance XP is scaled by the multiplier (headshot, distance, sneak, etc.).
        Bullseye_hit                = function(damageMult)
            onBullseyeHit(damageMult)
        end,
        -- OSSC (Oblivion-Style Spell Casting): a quick-cast that the player makes
        -- without entering a spell stance. OSSC sends OSSC_CastingState{isCasting}
        -- to the player on each cast (isCasting=true exactly once per real cast,
        -- past its own abort checks); we feed the spellcasting stance DIRECTLY so a
        -- weapon-stance hybrid still trains it. grantStanceXpDirect already enforces
        -- the 'xpOnSpellCast' toggle and the stance being enabled, so we only gate
        -- on the integration here. Inlined to avoid a top-level local. No OSSC edit
        -- is needed; absent OSSC, the event never fires and this is inert.
        OSSC_CastingState           = function(data)
            if type(data) ~= 'table' or data.isCasting ~= true then return end
            if not integrationEnabled('ossc') then return end
            local o = config.ossc or {}
            grantStanceXpDirect(tonumber(o.castXp) or 0.8, 'spell', o.castStance or 'arcanist')
        end,
