...
function M.new(ctx)
    ...
    local activeSmokeId    = nil     -- which pe_hackle-lo_smoke* is active (drives per-type bonus)
    ...
    -- Reverse of IMBUE_PREFIX_BY_EFFECT: the cached prefix back to its element
    -- effect id, for the Spellsword Resonance bonus (Phase 3).
    local PREFIX_TO_ELEMENT = {
        Blazed      = 'firedamage',
        Frozen      = 'frostdamage',
        Electrified = 'shockdamage',
    }
    -- Current Spellsword imbue element ('firedamage'/'frostdamage'/'shockdamage')
    -- or nil. Reads the cached prefix, so it costs nothing beyond the poll refresh.
    local function getImbueElement()
        return imbuePrefix and PREFIX_TO_ELEMENT[imbuePrefix] or nil
    end
    ...
    local function scanActiveSmoke()
        ...
        if not ok or not active then return false, 0, 0, nil end
        local smoking, speedDrain, remaining, chosenId = false, 0, 0, nil
        pcall(function()
            for _, params in pairs(active) do
                ...
                if type(id) == 'string' then
                    ...
                    if SMOKE_POTION_IDS[lid] then
                        ...
                        local thisRemaining = 0
                        if params.effects then
                            for _, eff in pairs(params.effects) do
                                local dl = eff and (eff.durationLeft or eff.timeLeft)
                                if type(dl) == 'number' and dl > thisRemaining then
                                    thisRemaining = dl
                                end
                            end
                        end
                        if thisRemaining > remaining then remaining = thisRemaining end
                        -- The dominant smoke (longest remaining; first found breaks
                        -- ties) decides the per-type bonus. With one smoke active —
                        -- the normal case — this is simply that smoke.
                        if chosenId == nil or thisRemaining >= remaining then
                            chosenId = lid
                        end
                    end
                end
        ...
        return smoking, speedDrain, remaining, chosenId
    end
    ...
    local function refreshSmoking()
        ...
        local smokeActive, speedDrain, remaining, smokeId = scanActiveSmoke()
        smokingActive = smokeActive
        activeSmokeId = smokeActive and smokeId or nil
    ...
    local function currentSmokingWeaponBonus()
        ...
        local s = config.smoker or {}
        -- Per-ingredient bonus: each smoke type the Hackle-Lo mod applies grants a
        -- different weapon-skill bonus (config.smoker.weaponBonusByType). Unlisted
        -- types fall back to the flat weaponBonus, so behaviour is unchanged unless
        -- a type is given its own value.
        local byType = s.weaponBonusByType
        if activeSmokeId and type(byType) == 'table' and byType[activeSmokeId] ~= nil then
            return tonumber(byType[activeSmokeId]) or 0
        end
        return tonumber(s.weaponBonus) or 10
    end

    -- Active smoke ingredient for UI consumers, or nil. { id, name, bonus } where
    -- `name` is the friendly leaf name (config.smoker.typeNames) and `bonus` is the
    -- weapon-skill bonus that ingredient is currently granting.
    local function getActiveSmokeInfo()
        if not smokeBonusLive or not activeSmokeId then return nil end
        local s = config.smoker or {}
        local names = (type(s.typeNames) == 'table') and s.typeNames or {}
        return {
            id    = activeSmokeId,
            name  = names[activeSmokeId] or 'Hackle-Lo',
            bonus = currentSmokingWeaponBonus(),
        }
    end
    ...
    local function getActivePrefixNotes(stanceId)
        ...
        if brawlerGauntletTier ~= 'none' then
            ...
            -- Iron Fist for OpenMW: a short confirmation that the (separately
            -- computed, victim.lua-side) amplification is currently live — no
            -- exact number here, since the real bonus also depends on the
            -- attack's own swing-strength variance, which only exists at the
            -- moment of an actual hit. Presence+enabled mirrors the same
            -- IronFistRuntime check victim.lua makes, so this note only shows
            -- when the bonus genuinely can fire.
            if integrationEnabled('ironfist') then
                local ifOk, ifPresent = pcall(function()
                    local s = storage.globalSection('IronFistRuntime')
                    return s and next(s:asTable()) ~= nil and s:get('enabled') ~= false
                end)
                if ifOk and ifPresent then
                    notes[#notes + 1] = 'Iron Fist: your gauntlets carry extra Brawler-trained force into every punch.'
                end
            end
        end
    ...
    return {
        ...
        getActiveSmokeInfo         = getActiveSmokeInfo,
        isSmokingActive            = isSmokingActive,
        -- Spellsword imbue element ('firedamage'/'frostdamage'/'shockdamage') or nil:
        getImbueElement            = getImbueElement,
        -- Brawler gauntlet tradeoff (hand-armor weight class while unarmed):
