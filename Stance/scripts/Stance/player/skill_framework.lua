...
function M.new(ctx)
    ...
    -- Active smoke ingredient { id, name, bonus } or nil, to name the leaf in the
    -- Smoking tooltip line (per-ingredient bonuses, §4).
    local getActiveSmokeInfo = ctx.getActiveSmokeInfo or function() return nil end
    -- Evening Star Tribunal worship bonus for the active stance's target skill
    -- (0 when off/not worshipping), and tooltip info { deityName, title,
    -- pantheonName, tierName, bonus } for the active stance (or nil).
    local worshipSkillBonus     = ctx.worshipSkillBonus     or function() return 0 end
    local getWorshipTooltipInfo = ctx.getWorshipTooltipInfo or function() return nil end
    ...
    -- Brawler gauntlet tier detection for dynamic description.
    ...
    -- Same instrument -> icon mapping as player/hud.lua's iconPathFor, kept
    -- in sync so the stats-menu/tooltip glyph always matches the HUD icon.
    ...
    local function computeBonusForSkill(skillId)
        ...
        -- Fold in the Evening Star worship bonus for the active stance's target
        -- skill (0 when off / not worshipping), so modded-skill stances (Twirler,
        -- Thaumaturge, Angler, Pitmen) receive the blessing too.
        return bonus + math.floor((worshipSkillBonus() or 0) + 0.5)
    end
    ...
    local function refreshEffectivenessModifiers()
        ...
        if effNeedsReconcile then
            ...
            if targetSkill then
                ...
                if stat then
                    ...
                    -- Cap at OUR full PERSISTENT contribution to this skill —
                    -- effectiveness PLUS any Evening Star worship bonus — so a
                    -- persisted modifier that already includes both is recognised as
                    -- ours and not re-added (which would otherwise double the worship
                    -- portion on every load). Smoking/gauntlet addends are transient
                    -- and 0 at load, so they need not be included here.
                    local ours = (targetBonus or 0) + math.floor((worshipSkillBonus() or 0) + 0.5)
                    local present = math.min(ours, math.max(0, actualMod))
        ...
        -- Evening Star Tribunal worship bonus for the active stance's target skill
        -- (0 when off / not worshipping). Added as an integer addend alongside the
        -- effectiveness and smoking bonuses, on the active stance's target skill only.
        local worshipBonus = math.floor((worshipSkillBonus() or 0) + 0.5)
        for _, sid in ipairs(VANILLA_EFF_SKILLS) do
            ...
            if sid == 'handtohand' then
                ...
                local worshipHh = (targetSkill == 'handtohand') and worshipBonus or 0
                contrib = math.floor(effPart + gauntletHhRaw + 0.5) + smokHh + worshipHh
            else
                contrib = (sid == targetSkill) and (targetBonus + smokingBonus + worshipBonus) or 0
    ...
    local function syncSfToActiveStance()
        ...
        -- Dynamic description for Brawler based on gauntlet tier
        local description = stance.description
        if stanceId == 'brawler' then
            local tier = getBrawlerGauntletTier()
            if tier == 'light' then
                description = 'The fist is the oldest weapon — swaddled in leather for precision strikes. Speed tempers the blow; agility carries the day.'
            elseif tier == 'medium' then
                description = 'The fist armored in steel — a balance between raw force and practiced technique. Weight and control trade blows against the relentless.'
            elseif tier == 'heavy' then
                description = 'The fist encased in plate — a sledge wrapped in gauntlets. Strength alone drives it; momentum becomes the message.'
            end
        end
        ...
        if readSetting('Tooltip', 'showMechanicTooltips', true) then
            ...
            -- Evening Star: the stance's Tribunal patron and, when the player
            -- worships that deity, the granted skill blessing and devotion tier.
            -- Shown only while the integration is enabled (nil otherwise).
            do
                local w = getWorshipTooltipInfo(stanceId)
                if w then
                    if (w.bonus or 0) > 0 and w.tierName then
                        table.insert(lines, string.format('%s: %s blesses this stance  (+%d %s, %s)',
                            w.pantheonName, w.deityName, w.bonus, skillLabel, w.tierName))
                    else
                        table.insert(lines, string.format('%s: sworn to %s, %s',
                            w.pantheonName, w.deityName, w.title))
                    end
                end
            end
            ...
            do
                ...
                if sbuff then
                    ...
                    local sinfo = getActiveSmokeInfo()
                    local leaf  = sinfo and sinfo.name
                    if leaf then
                        table.insert(lines, string.format('Smoking %s: +%d %s  (%d:%02d left)',
                            leaf, smag, skillLabel, smm, sss))
                    else
            ...
            if integrationPresent('bardcraft') then
                ...
                if buff then
                    ...
                    table.insert(lines, string.format('Song: +%d %s inspiration%s  (%d:%02d left)',
                ...
                if titles and #titles > 0 then
                    ...
                    table.insert(lines, string.format('Song titles: %s%s', table.concat(shown, ', '), more))
                end
                if stanceId == 'muse' then
                    ...
                    if ps then
                        ...
                        local target = ps.stance and formatStanceName(ps.stance) or '(unrecognised song)'
                        table.insert(lines, string.format("Performing: '%s'", ps.songTitle or '?'))
                        table.insert(lines, string.format('  Inspires: %s', target))
                        table.insert(lines, string.format('  Loop %d/%d    Notes %d/%d',
                            ps.loop or 0, ps.allowed or 1, ps.successes or 0, ps.notes or 0))
                        table.insert(lines, string.format('  Inspiration banked: %d:%02d', mm, ss))
                    else
